...
One window shows either the deployment the application carries or an existing one: `dsh-app://app/` is the bundled local deployment, whose Host runs on this machine and takes its model route from this machine's Harness home, while server mode loads an existing deployment's Web UI over HTTPS and lets that deployment own accounts, workspaces, sessions and the model route. Only server mode draws the shell-owned banner, whose shadow root the page cannot restyle, and that banner names the deployment and carries the single action returning to local mode — the absence of a banner is the local signal, and the window title names the mode either way. The caption keeps its Application and Edit menus in both modes — server mode mounts them itself, because a document this shell did not author publishes no overlay seat — and the banner's action opts out of the caption strip's drag region, without which a press would move the window instead of pressing the button.

The deployment offered comes from three layers, most specific first: `DSH_DESKTOP_SERVER_MODE` (the deployment object itself, as JSON), `<userData>/desktop-client.json` (`{ "server": { "origin": …, "label": …, "certificateSha256": … } }`), then `dshDesktopServerMode` in the application manifest — the value packaging bakes, so an installed client points at its deployment with no configuration. `desktop-mode.json` remembers whatever is chosen afterwards, an explicit local mode included, and the menu switches modes without restarting the Host — and a machine that has never chosen opens on the configured deployment, which is why an installed client shows that deployment's sign-in page rather than the bundled application.

Trust is scoped to the deployment's host and port — one listener serves the Web UI's document over HTTPS and its session stream over WSS — and decided per certificate error: with `certificateSha256` every other certificate is refused, and without it the deployment's own certificate is accepted and the fingerprint reported, for an operator to pin afterwards. Navigation stays inside that origin, because a document this shell did not author rewrites its own path freely, and every other destination opens in the default browser. The mode chosen, every certificate decision and every failed switch are appended to `desktop.log` beside the client settings file, which is what a machine showing the wrong document hands over.
